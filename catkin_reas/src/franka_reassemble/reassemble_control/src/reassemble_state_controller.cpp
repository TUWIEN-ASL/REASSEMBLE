...
#include <reassemble_control/reassemble_state_controller.h>
...
bool ReassembleStateController::init(hardware_interface::RobotHW* robot_hardware,
                                 ros::NodeHandle& root_node_handle,
                                 ros::NodeHandle& controller_node_handle) {
  franka_state_interface_ = robot_hardware->get<franka_hw::FrankaStateInterface>();
  if (franka_state_interface_ == nullptr) {
    ROS_ERROR("ReassembleStateController: Could not get Franka state interface from hardware");
    return false;
  }
  if (!controller_node_handle.getParam("arm_id", arm_id_)) {
    ROS_ERROR("ReassembleStateController: Could not get parameter arm_id");
    return false;
  }
  double publish_rate(30.0);
  if (!controller_node_handle.getParam("publish_rate", publish_rate)) {
    ROS_INFO_STREAM("ReassembleStateController: Did not find publish_rate. Using default "
                    << publish_rate << " [Hz].");
  }
  ...
  if (!controller_node_handle.getParam("joint_names", joint_names_) ||
    ...
    ROS_ERROR(
        "ReassembleStateController: Invalid or no joint_names parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  try {
    ...
    ROS_ERROR_STREAM("ReassembleStateController: Exception getting franka state handle: " << ex.what());
    return false;
  }
  ...
  publisher_external_wrench_base_.init(controller_node_handle, "F_ext_base", 1);
  ...
  {
    std::lock_guard<realtime_tools::RealtimePublisher<geometry_msgs::WrenchStamped>> lock(
        publisher_external_wrench_base_);
    publisher_external_wrench_base_.msg_.header.frame_id = arm_id_ + "_K";
    publisher_external_wrench_base_.msg_.wrench.force.x = 0.0;
    publisher_external_wrench_base_.msg_.wrench.force.y = 0.0;
    publisher_external_wrench_base_.msg_.wrench.force.z = 0.0;
    publisher_external_wrench_base_.msg_.wrench.torque.x = 0.0;
    publisher_external_wrench_base_.msg_.wrench.torque.y = 0.0;
    publisher_external_wrench_base_.msg_.wrench.torque.z = 0.0;
  }
  return true;
}

void ReassembleStateController::update(const ros::Time& time, const ros::Duration& /* period */) {
  if (trigger_publish_()) {
    robot_state_ = franka_state_handle_->getRobotState();
    publishFrankaStates(time);
    publishTransforms(time);
    publishExternalWrench(time);
    publishExternalWrenchBase(time);
    publishJointStates(time);
    sequence_number_++;
  }
}

void ReassembleStateController::publishFrankaStates(const ros::Time& time) {
...
void ReassembleStateController::publishJointStates(const ros::Time& time) {
...
void ReassembleStateController::publishTransforms(const ros::Time& time) {
...
void ReassembleStateController::publishExternalWrench(const ros::Time& time) {
...
void ReassembleStateController::publishExternalWrenchBase(const ros::Time& time) {
  if (publisher_external_wrench_base_.trylock()) {
    publisher_external_wrench_base_.msg_.header.frame_id = arm_id_ + "_link0";
    publisher_external_wrench_base_.msg_.header.stamp = time;
    publisher_external_wrench_base_.msg_.wrench.force.x = robot_state_.O_F_ext_hat_K[0];
    publisher_external_wrench_base_.msg_.wrench.force.y = robot_state_.O_F_ext_hat_K[1];
    publisher_external_wrench_base_.msg_.wrench.force.z = robot_state_.O_F_ext_hat_K[2];
    publisher_external_wrench_base_.msg_.wrench.torque.x = robot_state_.O_F_ext_hat_K[3];
    publisher_external_wrench_base_.msg_.wrench.torque.y = robot_state_.O_F_ext_hat_K[4];
    publisher_external_wrench_base_.msg_.wrench.torque.z = robot_state_.O_F_ext_hat_K[5];
    publisher_external_wrench_base_.unlockAndPublish();
  }
}

}  // namespace reassemble_control

PLUGINLIB_EXPORT_CLASS(reassemble_control::ReassembleStateController, controller_interface::ControllerBase)
