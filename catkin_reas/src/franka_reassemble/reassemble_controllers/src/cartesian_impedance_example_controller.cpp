...
bool CartesianImpedanceExampleController::init(hardware_interface::RobotHW* robot_hw,
  ...
  tau_d_publisher_ = node_handle.advertise<std_msgs::Float32MultiArray>("tau_d_topic", 1000);
...
void CartesianImpedanceExampleController::update(const ros::Time& /*time*/,
  ...
  // Publish tau_d as a Float32MultiArray
  std_msgs::Float32MultiArray tau_d_msg;
  tau_d_msg.data.clear();
  for (size_t i = 0; i < tau_d.size(); ++i) {
    tau_d_msg.data.push_back(tau_d[i]);
  }
  tau_d_publisher_.publish(tau_d_msg);
...
void CartesianImpedanceExampleController::complianceParamCallback(
  ...
  nullspace_stiffness_target_ = config.nullspace_damping;
}
