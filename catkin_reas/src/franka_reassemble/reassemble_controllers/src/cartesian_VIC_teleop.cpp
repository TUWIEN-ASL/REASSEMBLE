...
#include <franka_example_controllers/cartesian_VIC_teleop.h>
...
bool CartesianVICTeleop::init(hardware_interface::RobotHW* robot_hw,
                                               ros::NodeHandle& node_handle) {

  sub_equilibrium_pose_ = node_handle.subscribe(
      "equilibrium_pose", 20, &CartesianVICTeleop::equilibriumPoseCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());
  sub_teleop_pose_ = node_handle.subscribe(
      "teleop_pose", 20, &CartesianVICTeleop::teleopPoseCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());
  sub_alpha_virt_ = node_handle.subscribe(
      "alpha_virt", 20, &CartesianVICTeleop::alphaVirtCallback, this);
  sub_vel_d_ = node_handle.subscribe(
      "desired_velocity", 20, &CartesianVICTeleop::velDCallback, this);
  sub_wrench_d_ = node_handle.subscribe(
      "desired_wrench", 20, &CartesianVICTeleop::wrenchDCallback, this);
  sub_wrench_meas_ = node_handle.subscribe(
      "measured_wrench", 20, &CartesianVICTeleop::wrenchMeasCallback, this);
  tank_energy_pub_ = node_handle.advertise<std_msgs::Float64>("tank_energy", 1);

  std::string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR_STREAM("CartesianVICTeleop: Could not read parameter arm_id");
    return false;
  }
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "CartesianVICTeleop: Invalid or no joint_names parameters provided, "
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianVICTeleop: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianVICTeleop: Exception getting model handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianVICTeleop: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianVICTeleop: Exception getting state handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianVICTeleop: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "CartesianVICTeleop: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  cartesian_stiffness_teleop_.setZero();
  cartesian_stiffness_teleop_.topLeftCorner(3, 3)
          << 500 * Eigen::Matrix3d::Identity();
  cartesian_stiffness_teleop_.bottomRightCorner(3, 3)
          << 25 * Eigen::Matrix3d::Identity();
  cartesian_damping_teleop_.topLeftCorner(3, 3)
          << 2*sqrt(500) * Eigen::Matrix3d::Identity();
  cartesian_damping_teleop_.bottomRightCorner(3, 3)
          << 2*sqrt(25) * Eigen::Matrix3d::Identity();

  teleop_error_last_.setZero();
...
void CartesianVICTeleop::starting(const ros::Time& /*time*/) {
  ...
  teleop_position_d_ = initial_transform.translation();
  teleop_orientation_d_ = Eigen::Quaterniond(initial_transform.rotation());
  teleop_position_d_target_ = initial_transform.translation();
  teleop_orientation_d_target_ = Eigen::Quaterniond(initial_transform.rotation());

  // set nullspace equilibrium configuration to initial q
  q_d_nullspace_ = q_initial;
}

void CartesianVICTeleop::update(const ros::Time& /*time*/,
  ...
  error_teleop.head(3) << position - teleop_position_d_;
  if (teleop_orientation_d_.coeffs().dot(orientation.coeffs()) < 0.0) {
    orientation.coeffs() << -orientation.coeffs();
  }
  
  Eigen::Quaterniond error_quaternion_teleop(orientation.inverse() * teleop_orientation_d_);
  error_teleop.tail(3) << error_quaternion_teleop.x(), error_quaternion_teleop.y(), error_quaternion_teleop.z();
  error_teleop.tail(3) << -transform.rotation() * error_teleop.tail(3);

  // ROS_ERROR_STREAM("error_teleop: " << std::endl << error_teleop << std::endl);
  // ROS_ERROR_STREAM("teleop_error_last_: " << std::endl << teleop_error_last_ << std::endl);
  // ROS_ERROR_STREAM("period: " << std::endl << period.toSec() << std::endl);
  Eigen::Matrix<double, 6, 1> error_vel_teleop;
  error_vel_teleop << (error_teleop - teleop_error_last_) / (period.toSec() + 1e-10);
  // ROS_ERROR_STREAM("error_teleop_vel: " << std::endl << error_vel_teleop << std::endl);
  teleop_error_last_ = error_teleop;
  ...
  // compute control
  // allocate variables
  Eigen::VectorXd tau_task(7), tau_nullspace(7), tau_d(7), tau_FC(7), tau_teleop(7);
  ...
  // ROS_ERROR_STREAM("error_task: " << std::endl << error << std::endl);
  tau_task << jacobian.transpose() *
                  (-cartesian_stiffness_H_ * error - cartesian_damping_H_ * error_vel);


  tau_teleop << jacobian.transpose() *
                  (-cartesian_stiffness_teleop_ * error_teleop - cartesian_damping_teleop_ * error_vel_teleop);
  // ROS_ERROR_STREAM("Stifness: " << std::endl << cartesian_stiffness_teleop_ << std::endl);
  ...
  tau_nullspace << (Eigen::MatrixXd::Identity(7, 7) -
                    jacobian.transpose() * jacobian_transpose_pinv) *
                       (-(2.0 * sqrt(nullspace_stiffness_)) * dq);
  // Desired torque
  tau_d << (1-alpha_virt)*(tau_task + tau_FC) + alpha_virt * tau_teleop + tau_nullspace + coriolis;

  // ROS_ERROR_STREAM("tau_teleop: " << std::endl << tau_teleop << std::endl);
  // ROS_ERROR_STREAM("tau_task: " << std::endl << tau_task << std::endl);
  // ROS_ERROR_STREAM("tau_FC: " << std::endl << tau_FC << std::endl);
  ...
  alpha_virt = filter_params_ * alpha_virt_target + (1.0 - filter_params_) * alpha_virt;
  ...
  std::lock_guard<std::mutex> teleop_position_d_target_mutex_lock(
      teleop_position_and_orientation_d_target_mutex_);

  if(!was_zeroed)
  {
    teleop_position_d_target_ = teleop_position_d_;
    teleop_orientation_d_target_ = teleop_orientation_d_;
  }
  teleop_position_d_ = filter_params_ * teleop_position_d_target_ + (1.0 - filter_params_) * teleop_position_d_;
  teleop_orientation_d_ = teleop_orientation_d_.slerp(filter_params_, teleop_orientation_d_target_);
...
Eigen::Matrix<double, 7, 1> CartesianVICTeleop::saturateTorqueRate(
...
void CartesianVICTeleop::equilibriumPoseCallback(
...
void CartesianVICTeleop::teleopPoseCallback(
    const geometry_msgs::PoseStampedConstPtr& msg) {
  std::lock_guard<std::mutex> teleop_position_d_target_mutex_lock(
      teleop_position_and_orientation_d_target_mutex_);
  teleop_position_d_target_ << msg->pose.position.x, msg->pose.position.y, msg->pose.position.z;
  Eigen::Quaterniond teleop_last_orientation_d_target(teleop_orientation_d_target_);
  teleop_orientation_d_target_.coeffs() << msg->pose.orientation.x, msg->pose.orientation.y,
      msg->pose.orientation.z, msg->pose.orientation.w;
  if (teleop_last_orientation_d_target.coeffs().dot(teleop_orientation_d_target_.coeffs()) < 0.0) {
    teleop_orientation_d_target_.coeffs() << -teleop_orientation_d_target_.coeffs();
  }
}

void CartesianVICTeleop::alphaVirtCallback(const std_msgs::Float64::ConstPtr& msg) {
  this->alpha_virt_target = msg->data;

  if(alpha_virt_target > 0.95 && !was_zeroed){
    teleop_error_last_.setZero();
    was_zeroed = true;
  }

  if(alpha_virt_target < 0.05)
    was_zeroed = false;
}
void CartesianVICTeleop::alphaHCallback(const std_msgs::Float64::ConstPtr& msg) {
  alpha_h = msg->data;
...
void CartesianVICTeleop::velDCallback(
...
void CartesianVICTeleop::wrenchDCallback(
...
void CartesianVICTeleop::wrenchMeasCallback(
    const geometry_msgs::WrenchStampedConstPtr& msg) {
...
Eigen::VectorXd CartesianVICTeleop::updateEnergyTank(
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::CartesianVICTeleop,
